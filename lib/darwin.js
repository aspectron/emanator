...
class Darwin extends Core {
	...
	dmg_init(callback) {
		return new Promise(async (resolve,reject) => {
			...
			if(fs.existsSync(this.DMG)) {
				fse.emptyDirSync(this.DMG);
			}else
				fse.ensureDirSync(this.DMG)
			...
			async (err) => {
			    ...
			    await Promise.all(files);
			  	...
			  	fse.copySync(path.join(this.appFolder, resourceFolder, this.name+".icns"),path.join(NWJS_APP_RESOURCES,"app.icns"));
			  	fse.copySync(path.join(this.appFolder, resourceFolder, this.name+".icns"),path.join(NWJS_APP_RESOURCES,"document.icns"));

			  	gutil.log("Setting up background...");
			  	fse.ensureDirSync(path.join(this.DMG, '.background'));
			  	fse.copySync(path.join(this.appFolder, resourceFolder , this.name+".png"),path.join(this.DMG,'.background/'+this.name+'.png'));

			  	gutil.log("Symlink for /Applications...");
			  	fs.symlinkSync("/Applications", path.join(this.DMG, "/Applications"), 'dir');
		...
		return new Promise(async (resolve, reject) => {
			...
			this.spawn('hdiutil',
				[
					...
					`-fs`,`HFS+`,`-format`,`UDRW`,`${this.DMG_APP_NAME}.dmg`
				],
				//   if(v == "$FSARGS")
	...
	dmg_configure(callback) {
		...
		fs.writeFileSync(path.join(this.DEPS,'osa'), script);
	  	...
	  	this.spawn('osascript',"osa".split(" "), { cwd : this.DEPS, stdio : 'inherit' }, (err, code) => {
			err && console.log(err);
			if(code)
