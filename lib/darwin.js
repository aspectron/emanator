	...
	dmg_init(callback) {
		...
		fse.emptyDir(this.DMG, (err) => {
			...
			_.each([
				...
				path.join(this.ROOT, this.targetDMG)
				], (target) => {
					if(fs.existsSync(target)) {
			...
			fse.copy(path.join(this.BUILD,'nwjs.app'), path.join(this.ROOT,"DMG/"+this.identUCFC+".app"), (err) => {
				if(err)
					return callback(err);

				var NWJS_APP_RESOURCES = path.join(this.DMG, this.identUCFC+".app/Contents/Resources");
				...
				fse.copy(path.join(this.BUILD,'package.nw'), path.join(NWJS_APP_RESOURCES,"app.nw"), (err) => {
				  	...
				  	fse.copySync(path.join(this.appFolder,"resources/"+this.ident+"-dmg.png"),path.join(this.DMG,'.background/'+this.ident+'.png'));
