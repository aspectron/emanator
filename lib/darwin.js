...
class Darwin extends Core {
	...
	nwjs_unzip_symlink(callback) {
		if(this.PLATFORM != "darwin") {
			console.log("PLATFORM:"+this.PLATFORM+": Skipping nwjs-unzip-symlink...");
			return callback();
		}
		let folder = path.join(this.BUILD, 'nwjs.app');
	...
	nwjs_post_cleanup(callback) {
		...
		var attr = 751;
		var nwjsPath = path.join(this.BUILD,'nwjs.app/Contents/MacOS/nwjs');
	...
	dmg_unmount(callback) {
		...
		this.util.spawn('hdiutil',[`detach`,`/Volumes/${this.DMG_APP_NAME}`], { cwd : this.ROOT, stdio : 'inherit' }, (err, code) => {
			callback();
		})
	}
	...
	dmg_init(callback) {
		...
		fse.emptyDir(this.DMG, (err) => {
			...
			_.each([
				...
				path.join(this.RELEASE, this.targetDMG)
				], (target) => {
			...
			fse.copy(path.join(this.BUILD,'nwjs.app'), path.join(this.ROOT,"DMG/"+this.DMG_APP_NAME+".app"), (err) => {
				if(err)
					return callback(err);
				...
				fse.copy(path.join(this.BUILD,'package.nw'), path.join(NWJS_APP_RESOURCES,"app.nw"), (err) => {
				  	if(err)
				  		return callback(err);

				  	gutil.log("Setting up OSX icns...");
				  	fse.copySync(path.join(this.appFolder,"resources/"+this.name+"-icon.icns"),path.join(NWJS_APP_RESOURCES,"app.icns"));
				  	fse.copySync(path.join(this.appFolder,"resources/"+this.name+"-icon.icns"),path.join(NWJS_APP_RESOURCES,"document.icns"));

				  	gutil.log("Setting up background...");
				  	fse.ensureDirSync(path.join(this.DMG, '.background'));
				  	fse.copySync(path.join(this.appFolder,"resources/"+this.name+"-dmg.png"),path.join(this.DMG,'.background/'+this.name+'.png'));
	...
	dmg_create(callback) {
		...
		this.util.spawn('hdiutil',[`create`,`-volname`,`${this.DMG_APP_NAME}`,`-srcfolder`,`./DMG`,`-ov`,`-fs`,`HFS+`,`-format`,`UDRW`,`${this.ident}.dmg`],
	  //   if(v == "$FSARGS")
	...
	dmg_configure(callback) {
		...
		fs.writeFileSync(path.join(this.DEPS,'osa'), script);
	  	...
	  	this.util.spawn('osascript',"osa".split(" "), { cwd : this.DEPS, stdio : 'inherit' }, (err, code) => {
			err && console.log(err);
			if(code)
			...
			this.util.spawn('chmod',[`-R`,`a+rw`,`/Volumes/${this.DMG_APP_NAME}/${this.DMG_APP_NAME}.app`], (err, code) => {
				...
				var attr = 777; //751;
				var targets = [ ]
				var nwjsPath = `/Volumes/${this.DMG_APP_NAME}/${this.DMG_APP_NAME}.app/Contents/MacOS/nwjs`;
				targets.push(nwjsPath);
				//fs.chmodSync(nwjsPath, attr);
				var versions = `/Volumes/${this.DMG_APP_NAME}/${this.DMG_APP_NAME}.app/Contents/Versions`;
...
//		if(fs.existsSync(path.join(this.ROOT,this.targetDMG)))
//			fs.unlinkSync(path.join(this.ROOT,this.targetDMG));
		var args = (`convert -format UDZO -imagekey zlib-level=9 -o ../${this.targetDMG} ${this.ident}.dmg`).split(" ");
		gutil.log((`Converting and Compressing ${this.ident}.dmg to `).green+this.targetDMG.green.bold+"...".green, args);
		this.util.spawn('hdiutil', args, { cwd : this.ROOT, stdio : 'inherit' }, (err, code) => {
			err && console.log(err);
			if(code)
				return callback('Error running hdutil - code: '+code);
			...
			if(0)
				...
				fs.unlinkSync(path.join(this.ROOT,this.ident+".dmg"));
