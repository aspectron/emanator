...
class Darwin extends Core {
	constructor(appFolder, options) {
		...
		if(this.type.NWJS)
			this.tasks.platform.push('app-init', 'app-nwjs-renaming');
...
//		this.log("Darwin:tasks", this.tasks.platform)
	...
	manifest_read(...args){
		...
		this.targetDMG = `${this.ident}${this.suffix}-v${this.PROJECT_VERSION}-${this.PLATFORM_ARCH}.dmg`;
		this.packageJSON["product_string"] = this.DMG_APP_NAME.toLowerCase();
		return promise;
	}
	...
	app_init(callback) {
		return new Promise(async (resolve,reject) => {
			...
			this.NWJS_APP_CONTENTS = path.join(this.APP_DIR, this.APP_NAME, "/Contents");
			...
			this.log(`copying ${fileList.length} items...`);
			if(0) {
				...
				while(list.length) {
					let f = list.shift();
					//console.log('copy:',path.join(this.BUILD, f), '->', path.join(appNW, f));
					try {
						await this.copy(path.join(this.BUILD, f), path.join(appNW, f));
					} catch(ex) {
						console.log('error copying:',path.join(this.BUILD, f), '->', path.join(appNW, f));
			...
			if(this.manifest.aux_dmg_files && typeof this.manifest.aux_dmg_files == 'function') {
				this.log('executing aux dmg files');
				await this.manifest.aux_dmg_files(appNW);
			}

			if(this.type.NODE) {
				this.log('copying node binary');
...
CFBundleGetInfoString = "${this.DMG_APP_NAME} ${this.packageJSON.version}, Copyright ${(new Date()).getFullYear()} ${this.options.author}, The Chromium Authors, NW.js contributors, Node.js. All rights reserved.";
