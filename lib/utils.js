...
//				let archive = new AdmZip(path.join(folder,file));
			...
			else if(file.match(/\.tar(\.gz|\.xz)?$/ig)) {
				let folder = path.dirname(file);
				console.log('tar','-xvf',file,folder);
				try {
					await this.spawn('tar',['-xvf',file],{ cwd : folder, stdio:'inherit' });
				} catch(ex) {
	...
	zipFolder(folder,archive) {
		return this.spawn('zip',['-qdgds','10m','-r', `${archive}`, './'], {
			cwd : folder,
			stdio : 'pipe',
	...
	whereis(binary, binaryInResults = false) {
		let list = process.env.PATH.split(this.path_delimiter)
