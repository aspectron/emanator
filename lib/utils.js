...
class Utils {
	...
	async replicateFolder(root, pattern, destination, iterator) {
		let files = await this.glob(pattern, { cwd : root });
//		console.log("rp-",files);
		while(files.length) {
			let file = files.shift();
			let dirname = path.dirname(file);
			let basename = path.basename(file);
			await iterator(file, path.join(destination, dirname, basename));
		}
	}
