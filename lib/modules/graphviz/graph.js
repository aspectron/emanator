...
//console.log(containers);
        ...
        E.log(`graph: parsing ${files.length} files`);
...
*/
                ...
                CallExpression(node, st, c) {
                    ...
                    if(/\.(publish|subscribe|fetch|request|createPostgresIface)\(/.test(T(node))) {
                        ...
                        if(!subject) {
                            ...
                            let subject_text = T(subject);
                            let sanitized_subject = sanitize(subject_text);
        ...
        E.log(`graph: building references`);
...
//        }).join('\n');
        ...
        if(this.options?.graph) {
            custom = this.options.graph({
                refs, handlers : { pub, sub, req, fetch }, subjects : subject_map, targets
            });
        }
        ...
        let elements = targets.map(target => this.serialize(target.root));
...
//        console.log('links:',links.length);
        ...
        E.log(`graph: serialized ${elements.length} nodes with ${links.length} links`);
...
//        layout = "dot"
    ...
    subgraph cluster_root {
        ...
        fs.writeFileSync(path.join(folder,'graph.dot'), dot);

        E.log(`graph: compiling`);

        //await this.spawn('dot',['graph.dot','-Tsvg','-o','graph.svg','-Tpng','-o','graph.png','-Timap','-o','graph.map'], { cwd : __dirname, stdio : 'inherit' });
        let code = await this.spawn('dot',['graph.dot','-Tsvg','-o','graph.svg'], { cwd : folder, stdio : 'inherit' });
        // fs.unlinkSync(path.join(__dirname,'graph.dot'));
        if(code)
            console.log("dot exited with code:",code);

        let svg = fs.readFileSync(path.join(folder,'graph.svg'), { encoding : 'utf8' });
        ...
        fs.writeFileSync(path.join(folder,'graph.json'),JSON.stringify(nodes,null,'\t'));
...
// console.log(rank);
            ...
            return `${r.ident}  [label = "${r.label}", shape = "${r.shape||'box'}", color = "${this.color(r)}", style=filled, fillcolor=white];`;
        }
    }
}
