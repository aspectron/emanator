...
class Windows extends Core {
	...
	constructor(appFolder, options) {
		...
		if(this.flags.innosetup && !fs.existsSync(INNO_SETUP_COMPIL32)) {
		...
		fse.emptyDir(TEMP, (err) => {
			...
			fse.copy(path.join(__dirname,"../resources"),TEMP, { overwrite : true }, (err) => {
				...
				fse.copy(path.join(BUILD,'nw.exe'), path.join(TEMP,'nw.exe'), { overwrite : true }, async (err) => {
					...
					const RESOURCE_NAME = this.options?.resources?.ident || this.options?.resources?.name || MANIFEST_NAME;
					const RESOURCE_TARGET =  MANIFEST_NAME;


					// const RESOURCE_TARGET = MANIFEST_NAME;

					let rc = fs.readFileSync(path.join(__dirname,'../resources/','template.rc'),'utf8');
					rc = ejs.render(rc, { 
...
Log=CONSOLE
[COMMANDS]
-addoverwrite "<%- path.join(RESOURCE_FOLDER,resource+'.ico') %>", ICONGROUP,IDR_MAINFRAME,
`
console.log(rhs);
					const resourceFolder = this.options.resources?.folder || 'resources/setup'; // && this.options.resources.folder ? this.options.resources.folder : 'resources';
					if(!fs.existsSync(path.join(resourceFolder,`${RESOURCE_NAME}.ico`))) {
						console.log('Unable to locate',path.join(resourceFolder,`${RESOURCE_NAME}.ico`));
						process.exit(1);
					}
					rhs = ejs.render(rhs, { path, target : RESOURCE_TARGET, resource : RESOURCE_NAME, RESOURCE_FOLDER : path.join(this.appFolder,resourceFolder) });
					fs.writeFileSync(path.join(TEMP,'replace-icon.rhs'), rhs);
					console.log(this.RH,"-script replace-icon.rhs".split(' '));
	...
	setup(callback) {
		...
		return new Promise(async (resolve, reject) => {
			...
			let iss = fs.readFileSync(this?.options?.innosetup?.template || path.join(__dirname,'../resources/','template.iss'),'utf8');
			iss = ejs.render(iss, {
			...
			var file = path.join(this.SETUP,`${this.ident}${this.suffix}-v${this.packageJSON.version}-${this.PLATFORM_ARCH}.exe`);
			var stats = fs.lstatSync(file);
			this.setupFileSize_ = stats.size;
			this.log('Inno Setup:'.cyan.bold,file.toString().bold,'-',(stats.size.toFileSize()+'').magenta.bold);

			let hash = await this.utils.fileHash(file, 'sha1');
			let hashFile = file+'.sha1sum';
	...
	async pfx() {
		return new Promise(async (resolve,reject) => {
			...
			this.log('Signed Setup:'.cyan.bold,file.toString().bold,'-',(stats.size.toFileSize()+'').magenta.bold,'- Signature Size:',(sizeDiff.toFileSize()).magenta.bold);
