		...
		fse.emptyDir(TEMP, (err) => {
			...
			fse.copy(path.join(__dirname,"../resources"),TEMP, { overwrite : true }, (err) => {
				...
				fse.move(path.join(BUILD,'nw.exe'), path.join(TEMP,'nw.exe'), { overwrite : true }, (err) => {
					...
					fs.writeFileSync(path.join(this.TEMP,this.name+'.rc'), rc);
					
					this.util.spawn(this.RH,(`-open ${this.name}.rc -action compile -save ${this.name}.res -log CON`).split(' '), { cwd : TEMP, stdio : 'inherit' }, (err, code) => {
						if(!err && code)
							err = 'resource hacker (stage 0) failure; code: '+code;


						this.util.spawn(this.RH,(`-open nw.exe -resource ${this.name}.res -action addoverwrite -mask VERSIONINFO,,1033 -save ${this.name}.exe -log CON`).split(' '), { cwd : TEMP, stdio : 'inherit' }, (err, code) => {
							if(!err && code)
...
`
							...
							rhs = ejs.render(rhs, { path, name : this.name, RESOURCES : path.join(this.appFolder,'resources') });
	...
	setup(callback) {
		...
		if(this.flags.nopackage || this.options.nosetup)
			return callback();
	...
	pfx(callback) {
		...
		if(this.flags.nopackage || this.options.nosign)
			return callback();
