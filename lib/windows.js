...
class Windows extends Core {
	...
	resources(callback) {
		...
		let stdio = this.debug ? 'inherit' : 'ignore';
...
`
					...
					let resourceFolder = this.options.resources && this.options.resources.folder ? this.options.resources.folder : 'resources';
	...
	setup(callback) {
		return new Promise(async (resolve, reject) => {
			...
			let code = await this.utils.spawn('C:/Program Files (x86)/Inno Setup 5/compil32.exe',['/cc', path.join(this.TEMP,this.ident+'-impl.iss')], { cwd : this.ROOT, stdio : 'inherit' });
			if(code)
