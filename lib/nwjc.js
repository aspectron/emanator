...
//	HTML(...args) {
			...
			//try{
				let innerFn = (callback)=>{
					...
					if(this.core.flags.nonwjc) {
		 			...
		 			if(ext != '.js') {
						if(!fs.existsSync(folder)) {
							// this.log("...creating",folder.substring(this.core.RELEASE.length).yellow.bold);
							mkdirp.sync(folder);
						}
						...
						if(this.scripts[hash]) {
							...
							this.log("...skipping, storing RAW - No HTML ref:".red, src.substring(target.base.length).red.bold);
	...
	finish() {
		_.each(this.scripts, (script, hash) => {
			if(script.included)
				return;
			this.log("NWJC: ".cyan.bold+"HTML reference not found:".red.bold+' '+script.file);
		})
	}
		...
		_.each(deps, (o) => {
			...
			if(!fs.existsSync(absolute)) {
				...
				if(this.lastFault != title)
					this.log(title.yellow.bold+' - unable to locate: '.red.bold);
				this.lastFault = title;

				this.log('\t\t'.magenta.bold+file.magenta.bold);
			}
			else {
				let bin = '';
