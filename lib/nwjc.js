...
class NWJC {
	...
	HTML(...args) {
		...
		let fn = (target, enc, callback) => {
			...
			this.digestHTML_({ text, file, folder }, (err, text) => {
	...
	dest(destination) {
		...
		let fn = (target, enc, callback) => {
			...
			let folder = path.dirname(dest);
 			...
 			if(path.extname(dest).toLowerCase() != '.js') {
	...
	finish() {
		_.each(this.scripts, (script, hash) => {
			if(script.included)
				return;
			console.log("HTML reference not found:".red.bold, script.file);
		})
	}
	...
	NWJCompiler(src, dst, callback) {
		gutil.log(' +-:   '+src.substring(this.core.RELEASE.length).cyan);
		gutil.log(' +---> '+dst.substring(this.core.RELEASE.length).green);
