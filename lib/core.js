...
class Core {
	...
	constructor(appFolder, options) {
		...
		[
			'download', 'unzip', 'extract', 'spawn', 'exec',
		...
		// folders
		const folders = {
			RELEASE : this.options.BASE || path.join(os.homedir(),'emanator'),
			TOOLS : path.join(this.RELEASE,'tools'),
			DEPS : path.join(this.RELEASE,'deps'),
			SETUP : path.join(this.appFolder,'setup',this.PLATFORM_ARCH),
			ROOT : path.join(this.RELEASE,...rootFolder),
			TEMP : path.join(this.ROOT,'temp'),
			DMG : path.join(this.ROOT,"DMG"),
		}
		// this.SETUP = path.join(this.RELEASE,'setup');
		this.targetDMG = null;
		
		if(this.flags['release'])
			folders.REPO = path.join(this.ROOT,'repo');
		else
...
//				{ 'npm-install' : 'package-write' },
		...
		this.tasks.root = this.tasks.root.filter((task) => {
			if(_.isObject(task))
				return true;
			if(!this.type.NWJS && !this.args['with-sdk'] && task.match(/nwjs/ig))
				return false;
			return true;
		})
	...
	async generateTasks_() {
		...
		//console.log("DIGEST for".cyan.bold,ident.bold,"REMAINING:".yellow.bold,t);
			...
			this.options.debugTask && gutil.log(`${ident}::started...`.green.bold)
			if(!task) {
				this.options.debugTask && gutil.log(`${ident}::not found...`.red.bold, task)
				return cb();
			}

			let ts0 = Date.now();
			let handler = (err) => {
				this.options.debugTask &&  gutil.log(`${ident}::ended`.green.bold)
...
/////////////////////////////////////////////////////////////////////////////
	...
	async nwjs_sdk_unzip() {
		...
		let file = `nwjs-sdk-${this.NWJS_VERSION}-${this.NWJS_SUFFIX}-x64.${this.BINARIES_ARCHIVE_EXTENSION}`;
		await this.utils.unzip(path.join(this.DEPS,file), this.DEPS);
	}
