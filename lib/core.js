...
class Core {
	...
	constructor(appFolder, options) {
		...
		//this.BASE = path.join(appFolder, '..','release',this.ident);
		this.RELEASE = path.join(os.homedir(),'emanator'); //appFolder, '..','release');
		...
		this.flags = { }
		_.each(['init','reset','clean','force','release','rc','verbose','dbg','nonpm','nonwjc','fast','nopackage'], (v) => {
	...
	run() {
		...
		dpc(()=>{
			...
			Object.keys(this.options).map(k => [k,this.options[k]]).forEach((o) => {
				let [k,v] = o;
				console.log(`${k}:`.padStart(padding,' '),(v+'').bold);
			})
			console.log('');
...
//			console.log("DIGEST for".cyan.bold,ident.bold,"REMAINING:".yellow.bold,t);
			...
			let handler = (err) => {
				...
				//console.log(`+---> ${ident} [${(tdelta/1000).toFixed(2)} sec]`.green.bold,'...'+t.join(' '));
				digest(cb);
			}

			let progress = `...${((1.0-t.length/total)*100).toFixed(2)}% [${total-t.length}/${total}]`;

				gutil.log((ident+'...').padEnd(process.stdout.columns-11-progress.length)+progress.grey);//,'...'+t.join(' '));
//			console.log(`+-:`.bold+`   ${ident}`.bold);
			let fn = task.args.pop();
			let p = fn(handler);
			// if(p && typeof(p.then) == 'function') {
			// 	console.log(`awaiting ${ident}`);
			// 	await p;
			// 	console.log(`...${ident} done`);
			// 	dpc(()=>{
			// 		digest(cb);
			// 	})
			// }
...
//			throw new Error(`task() '${ident}' - 2nd argument (deps) must be an array of tasks`)
		...
		let wrap = async (...args) => {
			...
			args.push(closure);
			//fn.call(this, ...args);
			//console.log("CALLING",ident,args);
			// console.log("CALLING FN",fn);
			let p = fn.call(this, ...args);

			// p.then((v) => {
			// 	console.log("TESTING VALUE".magenta.bold,p,"V:",v);
			// })

			//console.log("TASK RETURN".blue.bold,ident.bold,p,JSON.stringify(p));
			// if(p)
			// 	console.log("p null",p == null,"p.then",p.then,'p.v',p.v);
			//if(ident == "::modules"){
	...
	init(callback) {
		console.log("CALLING INIT".red.bold);
...
//			let code = await this.utils.spawn('git',args, { cwd : this.ROOT, stdio });//, (err, code) => {
				// if(!err && code)
				// 	err = 'git error code: '+code;
				// if(err)
				// 	console.log(err);
...
//		});
	...
	//		this.util.spawn('tar',[archiveFileName+'tgz', '-cvfz' ]
			...
			_.each(strings, (v,k) => {
				...
				archive = archive.replace(regexp,v);
			})
	...
	//		let p = path.join(this.BUILD,archiveFile);
...
//7			let code = await this.utils.spawn('zip',['-qdd','-r', `${target}`, './'], {
