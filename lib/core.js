...
//				{ 'npm-install' : 'package-write' },
	...
	generateTasks_() {
		...
		function digest(cb) {
			...
			let handler = (err) => {
				if(err) {
					console.log(`Error while processing ${ident}`.red.bold);
					console.log(err);
...
//		this.projectName = this.pa
		...
		if(config) {
			console.log(`package.json 'gulp-config' property does not have an entry for`.magenta.bold, `${this.PLATFORM_ARCH}`.bold);
