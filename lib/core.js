...
const ARCH = (_ARCH_.arch == 'arm') ? _ARCH_+process.config.variables.arm_version : _ARCH_;
...
class Core {
	...
	constructor(appFolder, gulp, options) {
		...
		this.ARCHIVE = { windows : 'zip', darwin : 'zip', 'linux' : 'tar.gz' }[PLATFORM];
...
//				{ 'npm-install' : 'package-write' },
	...
	clone(callback) {
		...
		if(fs.existsSync(this.REPO)) {
			...
			this.util.spawn('git',args, { cwd : this.ROOT, stdio : 'inherit' }, (err, code) => {
				if(!err && code)
					err = 'git error code: '+code;
				if(err)
					console.log(err);
...
//		this.projectName = this.pa
		...
		if(!config)
			throw new Error(`package.json 'gulp-config' property does not have an entry for ${this.PLATFORM_ARCH}`)
	...
	nwjs_ffmpeg_copy(callback) {
		...
		switch(process.platform) {
			...
			case 'linux':{
				return this.gulp.src(path.join(this.DEPS,"libffmpeg.so"))
					.pipe(this.gulp.dest(path.join(this.BUILD,"lib")));
			}
			case 'darwin': {
				var versions = path.join(this.BUILD,'nwjs.app/Contents/Versions');
				var list = fs.readdirSync(versions);
				this.util.asyncMap(list, (v, next) => {
					fse.copy(
	...
	nwjs_sdk_download(callback) {
		let file = `nwjs-sdk-${this.NWJS_VERSION}-${this.NWJS_SUFFIX}-x64.${this.ARCHIVE}`
		let url = `https://dl.nwjs.io/${this.NWJS_VERSION}/${file}`;
		this.util.download(url,file,callback);
	}

	nwjs_download(callback) {
		let file = `nwjs-${this.NWJS_VERSION}-${this.NWJS_SUFFIX}-x64.${this.ARCHIVE}`
		let url = `https://dl.nwjs.io/${this.NWJS_VERSION}/${file}`;
		this.util.download(url,file,callback);
	}
	...
	nwjs_sdk_unzip(callback) {
		...
		this.util.unzip(file, (err)=>{
			...
			if(PLATFORM == "darwin") {
				...
				if(fs.existsSync(creditsHtml))
					fs.unlinkSync(creditsHtml);
