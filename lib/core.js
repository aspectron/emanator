...
class Core {
	...
	constructor(appFolder, options) {
		...
		this.modules = { }
		fs.readdirSync(path.join(__dirname,'modules')).forEach((t) => {
			t = t.replace(/\.js$/ig,'');
			this.modules[t] = require(`./modules/${t}`).Resolver(this);//{ go : Go.Resolver(this) }
		})
...
/////////////////////////////////////////////////////////////////////////////
	...
	nwjs_ffmpeg_copy(callback) {
		...
		switch(process.platform) {
			case 'win32' : {
				return this.copy(path.join(this.DEPS,"/ffmpeg*"),this.BUILD);
			} break;

			case 'linux':{
				return this.copy(path.join(this.DEPS,"libffmpeg.so"),path.join(this.BUILD,"lib"));
			}
			case 'darwin': {
