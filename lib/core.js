...
/*	fetchTask_2(ident) {
		if(!ident) {
			let k = Object.keys(this.registry).shift();
			if(!k)
				return null;
			return this.fetchTask_(k);
		}


		let task = this.registry[ident];
		if(!task) {
			console.log("no such task:",ident);
			return null;
		}

		if(task.deps_ && task.deps_.length) {
			if(this.rgistry[task.deps_[0]]) {
...
console.log(`...fetch: ${ident}`.bold)
		...
		if(task.deps_ && task.deps_.length) {
			let dep = task.deps_.shift();
			console.log('looking for'.bold,dep)
			let tdep = this.fetchTask_(dep);
			while(!tdep && task.deps_ && task.deps_.length) {
				dep = task.deps_.shift();
				tdep = this.fetchTask_(dep);
			}

			if(tdep && tdep.ident != dep)
				task.deps_.
		...
		console.log(('+'+task.ident).green.bold, task.deps);
		delete this.registry[task.ident];
		return task;
	}
*/
	...
	generateTasks_() {
		...
		function digest(cb) {
			...
			console.log("task".magenta.bold,task);
			if(!task) {
				console.log("no more tasks...".yellow.bold)
				return cb();
			}
			console.log(`starting ${task.ident}...`.yellow.bold);

			let handler = (err) => {

				console.log(`finished ${task.ident}...`);
...
//				dpc(()=>{ digest(cb);  });
			...
			//console.log("sssssss:", task.ident, p)
			//console.dir(p)

			//if(p && p.then) {
			//	console.log("PROMISE!!!".cyan.bold);
			//	promise.then(handler);
			//}
			//else {
				console.log("CALLBACK!!!".yellow.bold);
			//}
...
/*
		_.each(t,(k) => {
			let task = this.registry[k];
			if(!task) {
				console.log(`skipping ${k}...`.yellow.bold);
				return;
			}
			console.log(`adding ${task.ident}...`.green.bold,' -> '.bold, (task.args || '-').toString().yellow.bold,"   <==#");

//			this.gulp.task(task.ident, this.gulp.parallel(...task.args));
			// if(task.deps)
//			 	this.gulp.task(task.ident, this.gulp.parallel(...task.args));
			// else
			this.gulp.task(task.ident, this.gulp.series(...task.args));
			delete this.registry[k];
		})
*/


// 		let tasks = _.keys(this.registry);
// 		let ident = tasks.shift();
// 		let task = this.fetchTask_();
// 		while(task) {
// //			console.log("the task is:",task)
// //			console.log('task:'.green.bold,task.ident, task.deps);
// 			console.log(('#-task: '+task.ident).cyan.bold, '->', ...task.args )
// 			this.gulp.task(task.ident, this.gulp.series(...task.args));
// 			task = this.fetchTask_();
// 		}
	...
	task(ident, deps, fn) {
console.log("+-> queue:".yellow.bold,ident.bold);
		if(!this.sealing)
...
//		this.gulp.task(ident, this.gulp.series(...args));
	}
