...
class Core {
	...
	constructor(appFolder, options) {
		...
		// folders
		const RELEASE = this.options.BASE || path.join(os.homedir(),'emanator');
		const TOOLS = path.join(RELEASE,'tools');
		const DEPS = path.join(RELEASE,'deps');
		const SETUP = path.join(appFolder,'setup',this.PLATFORM_ARCH);
...
//				{ 'npm-install' : 'package-write' },
		...
		this.tasks.root = this.tasks.root.filter((task) => {
			if(_.isObject(task))
				return true;
			if(!this.type.NWJS && !this.flags['with-sdk'] && task.match(/nwjs/ig))
				return false;
			return true;
		})
