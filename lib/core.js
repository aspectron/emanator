...
class Core {
	...
	constructor(appFolder, options) {
		...
		if(options.innosetup && PLATFORM == "windows" && !fs.existsSync("C:/Program Files (x86)/Inno Setup 5/compil32.exe")) {
		...
		this.SETUP = path.join(this.appFolder,'setup',this.PLATFORM_ARCH);
		// this.SETUP = path.join(this.RELEASE,'setup');
		this.ROOT = path.join(this.RELEASE,...rootFolder);
		...
		if(this.flags['release'])
			...
			this.REPO = path.join(this.appFolder);
	...
	async run_(list_) {
		...
		dpc(()=>{
			...
			Object.keys(this.options).map(k => [k,this.options[k]]).forEach((o) => {
				let [k,v] = o;
				this.log(`${k}:`.padStart(padding+1,' '),(v+'').bold);
			})
			this.log('');
...
//			console.log("DIGEST for".cyan.bold,ident.bold,"REMAINING:".yellow.bold,t);
	...
	init(callback) {
		[this.RELEASE,this.TOOLS,this.ROOT,this.SETUP,this.DEPS,this.TEMP].forEach((folder) => {
			mkdirp.sync(folder);  
		});
		callback();
	}
...
//		console.log(process.stdout);
	...
	manifest_read(callback) {
		...
		if(this.flags['local-binaries'])
			...
			this.BIN = path.join(this.BUILD,'bin',this.PLATFORM_ARCH);


		[this.BUILD,this.PACKAGE].forEach((folder) => {
			mkdirp.sync(folder);  
		});
...
*/
	...
	async npm_install() {
		if(this.options.nonpm || this.flags.nonpm || this.flags['dry-run'])
			return;
		//return 
		return this.utils.spawn(this.NPM,['install'], { cwd : this.PACKAGE, stdio : 'inherit' });
		//callback();
	}
...
//		});
	...
	node_binary(callback) {
		if(!this.options.standalone || !this.type.NODE)
			return callback();
...
console.log("COPYING NODE BINARY",this.options);
    	...
    	fse.copy(process.argv[0], path.join(this.PACKAGE,file), callback);
	}
    ...
    async uploadFiles_(hashFile, setupFile) {
        ...
        return this.utils.spawn('scp', args, { cwd : this.SETUP, stdio : 'inherit' }, function(err, code) {
            if(err) {
                console.log(err);
                process.exit(code);
            }else{
	...
	async scp(dest) {
        ...
        return this.utils.spawn('scp', [this.ARCHIVE, dest], { cwd : this.SETUP, stdio : 'inherit' }, function(err, code) {
            if(err) {
                console.log(err);
                process.exit(code);
            }else{
	...
	//		this.util.spawn('tar',[archiveFileName+'tgz', '-cvfz' ]
			...
			if(archive === true)
				archive = '$IDENT-v$VERSION-$PLATFORM-$ARCH';
